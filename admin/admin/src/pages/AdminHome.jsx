...
const AdminHome = () => {
  ...
  const cards = [
    { label: 'Total E-waste Collection Points', key: 'totalReports', sub: 'Active 190 · Inactive 10', valuePrefix: '', color: 'green' },
    { label: 'Total Users', key: 'totalUsers', sub: '+6.5% Since Last Month', valuePrefix: '', color: 'mint' },
    { label: 'Total Reports', key: 'totalReports', sub: '+3.5% Since Last Month', valuePrefix: '', color: 'mint' },
    { label: 'Pending Reports', key: 'pendingReports', sub: '+2.4% Since Last Month', valuePrefix: '', color: 'mint' },
  ];
  ...
  return (
    <div>
      <h1 className="topbar-title" style={{ marginBottom: 16 }}>Welcome, Admin</h1>
      ...
      <div className="card" style={{ marginTop: 16 }}>
        ...
        <div className="card-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))' }}>
          {quickActions.map((action) => (
            <div key={action} className="card" style={{ boxShadow: 'none', border: '1px dashed #e5e7eb' }}>
              {action}
            </div>
          ))}
        </div>
