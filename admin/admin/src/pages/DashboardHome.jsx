...
const DashboardHome = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-800">Welcome, Admin</h1>
      </div>
      ...
      <div className="grid grid-cols-5 gap-4">
        <div className="bg-card rounded-2xl shadow-soft px-5 py-4">
          <p className="text-xs text-slate-500">Total E-waste Collection Points</p>
        ...
        <div className="bg-card rounded-2xl shadow-soft px-5 py-4">
          ...
          <p className="mt-3 text-[11px] text-emerald-500">↑ 6.5% Since Last Month</p>
        </div>
        ...
        <div className="bg-card rounded-2xl shadow-soft px-5 py-4">
          ...
          <p className="mt-3 text-[11px] text-emerald-500">↑ 3.5% Since Last Month</p>
        </div>
        ...
        <div className="bg-card rounded-2xl shadow-soft px-5 py-4">
          ...
          <p className="mt-3 text-[11px] text-emerald-500">↑ 2.4% Since Last Month</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-card rounded-2xl shadow-soft px-5 py-4 flex flex-col col-span-3">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs text-slate-500">Eco Points Balance Distribution</p>
            ...
            {ecoPointsData.length > 0 ? (
              (() => {
                const maxValue = Math.max(
                  ...ecoPointsData.map((d) => Math.max(d.positive || 0, d.negative || 0)),
                  1
                );
                const maxHeight = 120;
                return ecoPointsData.map((day, i) => {
                  const positiveHeight = maxValue > 0 ? (day.positive / maxValue) * maxHeight : 0;
                  const negativeHeight = maxValue > 0 ? (day.negative / maxValue) * maxHeight : 0;
                  const date = new Date(day.date);
                  const dayLabel = date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
                  return (
                    <div key={day.date || i} className="flex flex-col items-center flex-1 gap-1">
                      <div className="w-full rounded-t-lg bg-emerald-400" style={{ height: `${Math.max(positiveHeight, 4)}px` }}></div>
                      <div className="w-full rounded-t-lg bg-red-400" style={{ height: `${Math.max(negativeHeight, 4)}px` }}></div>
              ...
              <div className="w-full text-center text-xs text-slate-400 py-8">No data available</div>
            )}
          </div>
        </div>
          ...
          <div className="flex items-center gap-6 mt-2">
            <div className="space-y-1 text-[11px] flex-1">
              <div className="flex items-center justify-between">
                ...
                <span className="text-slate-500">{stats?.pendingReports || 0}</span>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                ...
                <span className="text-slate-500">{stats?.verifiedReports || 0}</span>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                ...
                <span className="text-slate-500">{stats?.collectedReports || 0}</span>
              </div>
            </div>
            <div className="relative w-32 h-32">
              ...
              <div className="absolute inset-3 rounded-full bg-card flex flex-col items-center justify-center text-[11px]">
                ...
                <span className="text-lg font-semibold text-slate-800">{stats?.totalReports || 0}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-6 gap-4">
        <div className="rounded-2xl shadow-soft bg-gradient-to-t from-emerald-600 to-emerald-500 px-4 py-3 text-white">
          ...
          <p className="mt-2 text-xl font-semibold">{stats?.collectedReports || 0}</p>
          <p className="mt-1 text-[11px] opacity-90">↑ 20%</p>
        </div>
        <div className="rounded-2xl shadow-soft bg-gradient-to-t from-emerald-600 to-emerald-500 px-4 py-3 text-white">
          <p className="text-[11px] opacity-90">Pending Reports</p>
          <p className="mt-2 text-xl font-semibold">{stats?.pendingReports || 0}</p>
        ...
        <div className="rounded-2xl shadow-soft bg-gradient-to-t from-rose-500 to-rose-400 px-4 py-3 text-white">
          ...
          <p className="mt-2 text-xl font-semibold">{stats?.cancelledReports || 0}</p>
          <p className="mt-1 text-[11px] opacity-90">↓ 10%</p>
        </div>
        <div className="rounded-2xl shadow-soft bg-gradient-to-t from-emerald-600 to-emerald-500 px-4 py-3 text-white">
          <p className="text-[11px] opacity-90">Verified</p>
          <p className="mt-2 text-xl font-semibold">{stats?.verifiedReports || 0}</p>
          <p className="mt-1 text-[11px] opacity-90">↑ 20%</p>
        </div>
        <div className="rounded-2xl shadow-soft bg-gradient-to-t from-emerald-600 to-emerald-500 px-4 py-3 text-white">
        ...
        <div className="rounded-2xl shadow-soft bg-gradient-to-t from-emerald-600 to-emerald-500 px-4 py-3 text-white">
          ...
          <p className="mt-2 text-xl font-semibold">{stats?.feedbackCount || 0}</p>
